...
export const BootSequence = ({ onComplete }: { onComplete: () => void }) => {
    ...
    return (
        <>
            <style>{`
                /* The artwork and the progress bar have to scale as one unit: the bar
                   is placed as a percentage of this stage, and the stage always keeps
                   the 1366x768 artwork's aspect ratio, so the bar stays tucked under
                   the logo on any screen shape. Below lg the stage is zoomed past the
                   viewport width — the artwork's backdrop is flat black, so the sides
                   that get cropped never show. */
                .boot-stage {
                    position: relative;
                    /* the centering flex parent would otherwise shrink the zoom away */
                    flex: none;
                    aspect-ratio: 1366 / 768;
                    /* the vh pair is the fallback for browsers without dvh */
                    width: min(150vw, calc(100vh * 1366 / 768));
                    width: min(150vw, calc(100dvh * 1366 / 768));
                }

                @media (min-width: 1024px) {
                    .boot-stage {
                        width: min(100vw, calc(100vh * 1366 / 768));
                        width: min(100vw, calc(100dvh * 1366 / 768));
                    }
                }
